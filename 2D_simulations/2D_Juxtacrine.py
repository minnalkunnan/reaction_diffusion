...
def neighbor_sum(Z):
    """
    Sum of 4-neighbour values (up, down, left, right) with periodic BCs.
    """
    return (
        np.roll(Z, 1, axis=0) +   # up
        np.roll(Z, -1, axis=0) +  # down
        np.roll(Z, 1, axis=1) +   # left
        np.roll(Z, -1, axis=1)    # right
    )
